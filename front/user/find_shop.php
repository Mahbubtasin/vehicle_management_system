...
$result = $db->query("SELECT * FROM `shop_owner` WHERE `usertype` = 'shop_owner'");

// Fetch the info-window data from the database
$result2 = $db->query("SELECT * FROM `shop_owner` WHERE `usertype` = 'shop_owner'");
?>
        ...
        function initMap() {
            ...
            var markers = [
                ...
                while($row = $result->fetch_assoc()){
                    echo '["'.$row['shop_name'].'", '.$row['lat'].', '.$row['lng'].'],';
                }
            }
                ?>
            ];
            ...
            var infoWindowContent = [
                ...
                '<h3><a href="shop.php?id=<?= $row['shop_owner_id']; ?>"><?php echo $row['shop_name']; ?></a></h3>' +
